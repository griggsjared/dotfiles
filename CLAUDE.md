...
**Always edit files in `/Users/jared/.dotfiles/`, never through their symlinked system locations.**
...
If you open a file via its system path (e.g. `~/.config/nvim/init.lua`), you are editing the symlink target, which is the dotfiles file — but prefer to explicitly use the dotfiles path to make it clear where the source of truth lives.
...
Run `stow` from the repo root (`/Users/jared/.dotfiles`).
