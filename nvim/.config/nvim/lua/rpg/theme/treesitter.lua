local M = {}

function M.get(c, _)
	local spec = {
		["@annotation"] = { fg = c.base.blue, italic = false },
		["@attribute"] = { fg = c.base.green },
		["@boolean"] = { fg = c.base.magenta },
		["@constant"] = { fg = c.base.magenta },
		["@constant.builtin"] = { fg = c.base.magenta },
		["@constant.macro"] = { fg = c.base.magenta },
		["@constructor"] = { fg = c.base.green },
		["@field"] = { fg = c.base.red },
		["@diff.delta"] = { fg = c.base.yellow },
		["@diff.minus"] = { fg = c.base.red },
		["@diff.plus"] = { fg = c.base.green },
		["@function"] = { fg = c.base.green },
		["@function.builtin"] = { fg = c.base.green },
		["@function.call"] = { fg = c.base.green },
		["@function.macro"] = { fg = c.base.green },
		["@function.method"] = { fg = c.base.green },
		["@function.method.call"] = { fg = c.base.green },
		["@keyword"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.conditional"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.coroutine"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.debug"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.directive"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.directive.define"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.exception"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.function"] = { fg = c.base.magenta, italic = true, bold = true },
		["@keyword.import"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.operator"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.repeat"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.return"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.storage"] = { fg = c.base.red, italic = true, bold = true },
		["@keyword.type"] = { fg = c.base.magenta, italic = true, bold = true },
		["@number"] = { fg = c.base.magenta },
		["@number.float"] = { fg = c.base.magenta },
		["@operator"] = { fg = c.base.red },
		["@variable"] = { fg = c.base.foreground },
		["@variable.builtin"] = { fg = c.base.blue, italic = true },
		["@variable.member"] = { fg = c.base.foreground },
		["@variable.parameter"] = { fg = c.base.cyan, italic = true },
		["@variable.parameter.builtin"] = { fg = c.base.cyan, italic = true },
		["@punctuation.bracket"] = { fg = c.base.red },
		["@punctuation.delimiter"] = { fg = c.base.dimmed2 },
		["@punctuation.special"] = { fg = c.base.dimmed2 },
		["@string"] = { fg = c.base.yellow },
		["@string.documentation"] = { fg = c.base.dimmed3 },
		["@string.escape"] = { fg = c.base.magenta },
		["@string.regexp"] = { fg = c.base.yellow },
		["@character"] = { fg = c.base.yellow },
		["@character.printf"] = { fg = c.base.yellow },
		["@character.special"] = { fg = c.base.yellow },
		["@tag"] = { fg = c.base.red },
		["@tag.attribute"] = { fg = c.base.blue, italic = true },
		["@tag.builtin"] = { fg = c.base.red },
		["@tag.delimiter"] = { fg = c.base.dimmed2 },
		["@markup"] = { fg = c.base.foreground },
		["@markup.emphasis"] = { fg = c.base.foreground, italic = true },
		["@markup.environment"] = { fg = c.base.foreground },
		["@markup.environment.name"] = { fg = c.base.foreground },
		["@markup.heading"] = { fg = c.base.green, bold = true },
		["@markup.italic"] = { fg = c.base.foreground, italic = true },
		["@markup.link"] = { fg = c.base.blue, underline = true },
		["@markup.link.label"] = { fg = c.base.blue, underline = true },
		["@markup.link.label.symbol"] = { fg = c.base.blue, underline = true },
		["@markup.link.url"] = { fg = c.base.blue, underline = true },
		["@markup.list"] = { fg = c.base.foreground },
		["@markup.list.checked"] = { fg = c.base.foreground },
		["@markup.list.markdown"] = { fg = c.base.foreground },
		["@markup.list.unchecked"] = { fg = c.base.foreground },
		["@markup.math"] = { fg = c.base.yellow },
		["@markup.raw"] = { fg = c.base.yellow },
		["@markup.raw.markdown_inline"] = { fg = c.base.yellow },
		["@markup.strikethrough"] = { fg = c.base.foreground, strikethrough = true },
		["@markup.strong"] = { fg = c.base.foreground, bold = true },
		["@markup.underline"] = { fg = c.base.foreground, underline = true },
		["@type"] = { fg = c.base.blue },
		["@type.builtin"] = { fg = c.base.blue, italic = true },
		["@type.definition"] = { fg = c.base.green },
		["@type.qualifier"] = { fg = c.base.blue },
		["@module"] = { fg = c.base.blue },
		["@module.builtin"] = { fg = c.base.blue },
		["@namespace.builtin"] = { fg = c.base.blue },
		["@label"] = { fg = c.base.blue },
		["@constant.cpp"] = { fg = c.base.blue },
		["@constant.macro.cpp"] = { fg = c.base.red },
		["@keyword.cpp"] = { fg = c.base.blue, italic = false },
		["@namespace.cpp"] = { fg = c.base.green },
		["@operator.cpp"] = { fg = c.base.red },
		["@punctuation.delimiter.cpp"] = { fg = c.sideBar.foreground },
		["@type.cpp"] = { fg = c.base.blue, italic = false },
		["@variable.cpp"] = { fg = c.base.foreground },
		["@function.call.bash"] = { fg = c.base.green },
		["@keyword.dockerfile"] = { fg = c.base.red },
		["@lsp.type.class.dockerfile"] = { fg = c.base.blue },
		["@parameter.bash"] = { fg = c.base.foreground },
		["@module.go"] = { fg = c.base.foreground },
		["@string.escape.go"] = { fg = c.base.magenta },
		["@function.macro.latex"] = { fg = c.base.green },
		["@punctuation.special.latex"] = { fg = c.base.red },
		["@string.latex"] = { fg = c.base.blue },
		["@text.emphasis.latex"] = { italic = true },
		["@text.environment.latex"] = { fg = c.base.green },
		["@text.environment.name.latex"] = { fg = c.base.blue, italic = true },
		["@text.math.latex"] = { fg = c.base.magenta },
		["@text.strong.latex"] = { bold = true },
		["@conceal.markdown"] = { bg = c.base.dark1 },
		["@markup.italic.markdown_inline"] = { italic = true },
		["@markup.link.label.markdown_inline"] = { fg = c.base.red },
		["@markup.link.url.markdown_inline"] = { fg = c.base.green, underline = true },
		["@markup.raw.block.markdown"] = { bg = c.base.dark1 },
		["@markup.raw.delimiter.markdown"] = { bg = c.base.dark1, fg = c.base.dimmed2 },
		["@markup.strong.markdown_inline"] = { bold = true },
		["@none.markdown"] = { bg = c.base.dark1 },
		["@punctuation.special.markdown"] = { fg = c.base.dimmed2 },
		["@text.emphasis.markdown_inline"] = { fg = c.base.foreground, italic = true },
		["@text.literal.block.markdown"] = { bg = c.editor.background },
		["@text.literal.markdown_inline"] = { bg = c.base.dimmed4, fg = c.base.foreground },
		["@text.quote.markdown"] = { bg = c.base.dimmed5, fg = c.base.foreground },
		["@text.reference.markdown_inline"] = { fg = c.base.red },
		["@text.strong.markdown_inline"] = { bold = true },
		["@text.uri.markdown_inline"] = { fg = c.base.green, sp = c.base.green, underline = true },
		["@function.scss"] = { fg = c.base.blue },
		["@keyword.scss"] = { fg = c.base.red },
		["@number.scss"] = { fg = c.base.magenta },
		["@property.scss"] = { fg = c.base.green },
		["@string.scss"] = { fg = c.base.blue, italic = true },
		["@type.scss"] = { fg = c.base.blue },
		["@comment.documentation.lua"] = { fg = c.base.blue },
		["@conditional.lua"] = { fg = c.base.red },
		["@field.lua"] = { fg = c.base.foreground },
		["@function.builtin.lua"] = { fg = c.base.green },
		["@keyword.function.lua"] = { fg = c.base.red, bold = true, italic = false },
		["@keyword.lua"] = { fg = c.base.red, italic = true, bold = true },
		["@namespace.lua"] = { fg = c.base.red },
		["@parameter.lua"] = { fg = c.base.cyan, italic = true },
		["@variable.lua"] = { fg = c.base.foreground },
		["@number.yaml"] = { fg = c.base.magenta },
		["@property.yaml"] = { fg = c.base.red },
		["@punctuation.special.yaml"] = { fg = c.base.foreground },
		["@string.yaml"] = { fg = c.base.yellow },
	}

	for i = 1, 9 do
		spec["@markup.heading." .. i .. ".markdown"] = { fg = c.base.yellow }
		spec["@markup.heading." .. i .. ".marker.markdown"] = { fg = c.base.dimmed2 }
	end

	return spec
end

return M
